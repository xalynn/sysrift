...
|---|--------|---------------|
| 1 | System Information | Hostname, identity, kernel version, interesting groups, env vars, PATH hijacking, kernel CVEs (DirtyCow, Dirty Pipe, eBPF) with distro backport detection (dpkg/rpm package version comparison against tracker-verified patch floors, derivative distro resolution, upstream fallback with qualifier), mount option analysis (nosuid/noexec/nodev on key paths), unmounted fstab entries, fstab credential detection |
