...
| 4 | Credential Hunting | History files, config file credentials, format-based secret scanning (AWS/GCP/GitHub/GitLab/Slack/SSH keys), shadow/passwd hashes, AuthorizedKeysFile analysis, PAM/LDAP creds, cached Kerberos/SSSD/Samba tickets, TTY audit harvesting, GitLab/Splunk/Log4j, database credential files (Redis/MySQL/PostgreSQL/MongoDB), mail spool readability, browser credential stores (Firefox/Chrome/Chromium/Brave/Vivaldi/Edge/Opera), container pivot annotation on SSH keys |
| 5 | Writable Files & Dirs | /etc/passwd, shadow, sudoers, ld.so.preload, binfmt_misc register, ld.so.conf library paths, profile.d scripts, logrotate abuse (logrotten race), non-standard ACL detection, world-writable directories |
