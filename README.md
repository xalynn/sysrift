...
|---|--------|---------------|
| 1 | System Information | Hostname, identity, kernel version, interesting groups (10 entries: sudo, docker, lxd, lxc, disk, adm, shadow, wheel, wireshark, kvm), env vars, PATH hijacking (writable dirs + broken symlinks in writable PATH dirs), interpreter library path hijacking (PYTHONPATH, RUBYLIB, PERL5LIB, NODE_PATH writable dir detection), kernel CVEs (15 entries: DirtyCow, eBPF ALU32, Dirty Pipe, OverlayFS FUSE, nf_tables OOB, nf_tables UAF, GameOverlay x2, user namespace ID map, PTRACE_TRACEME, netfilter setsockopt, legacy_parse_param, cgroup release_agent, cls_route UAF, nf_tables set UAF) with distro backport detection (dpkg/rpm package version comparison against tracker-verified fixed versions, derivative distro resolution, distro-gated Ubuntu-only entries, upstream fallback with qualifier), mount option analysis (nosuid/noexec/nodev on key paths), unmounted fstab entries, fstab credential detection |
...
| 4 | Credential Hunting | History files (deduplicated with repeat counts), config file credential patterns (with false positive filtering: sentinel values, .NET assembly metadata, ImageMagick templates, file size cap 256KB, line length cap 500 chars), JS/JSON scanning limited to web deploy dirs (/var/www, /srv, /opt), format-based secret scanning (7 patterns: AWS access keys, GCP service account JSON, GitHub classic + fine-grained PATs, GitLab PATs, Slack tokens, embedded private key headers — grep pre-filter + Crystal regex validation, same file size/line caps), /etc/shadow readability, /etc/passwd hashes, SSH keys (ownership-aware severity: own keys info, others' keys critical), non-standard AuthorizedKeysFile path analysis (%h/%u expansion, writable = inject key), .netrc, cloud credentials, PAM credential extraction (passwd=, bindpw, ldap_bind_pw, secret= in pam.d + standalone LDAP/PAM configs), cached credentials & tickets (Samba TDB, Quest VAS, SSSD caches/secrets, Kerberos keytabs + ticket caches, opasswd — label split: offline crackable vs usable for authentication), TTY audit password harvesting (aureport --tty gated behind auditd detection, streaming read, audit.log fallback), software-specific credentials (GitLab config secrets, Splunk obfuscated passwords, Log4j jar version detection < 2.17.1) |
| 5 | Writable Files & Dirs | High-value writable files (/etc/passwd, /etc/shadow, /etc/sudoers, /etc/ld.so.preload, etc.), binfmt_misc register writability (credentials flag → root execution), ld.so.conf recursive library path writability (include dirs, conf files, library dirs, ld.so.preload entries), /etc/profile.d/ writable script detection, world-writable directories |
| 6 | Network Information | Interfaces, routes, listening ports (flags databases, Docker API, admin panels, K8s, lateral movement targets), /etc/hosts, ARP, connections, forwarding, legacy r-commands trust (hosts.equiv/shosts.equiv wildcard detection, per-user .rhosts, r-service port 512-514 listeners via /proc/net/tcp, inetd/xinetd config scanning), firewall configuration (iptables/nftables/UFW/firewalld saved rules via file reads, kernel table presence via /proc/net/ip_tables_names, no-firewall detection, zero spawns) |
| 7 | Processes, Cron & Timers | Root processes with writable binaries, crontab analysis, cron wildcard injection (tar, chown, chmod), cron target binary writability (/dev/null filtered, directory-type filtered, user-aware severity: root cron = critical, non-root = medium), suspicious process location detection (self-filtered), systemd timers, process surface analysis: chroot jail detection (/proc/[pid]/root with SUID cross-reference), open FD analysis (sensitive extensions in sensitive dirs held by other users), /proc/*/environ credential harvesting (SENSITIVE_ENV_RE filtered, uid:key deduped, value-masked) |
...
| 11 | Installed Software | Compilers, interpreters, transfer tools, package counts, web servers, internal service detection (8 services: Gitea, Gogs, GitLab, Jenkins, Grafana, Vault, Consul — process + listener cross-reference), known vulnerable software (screen, pkexec PwnKit, polkit CVE-2021-3560, Exim), userspace CVEs (4 entries: Firejail CVE-2022-31214, needrestart CVE-2024-48990, glibc Looney Tunables CVE-2023-4911, apport-cli CVE-2023-1326 — distro-gated with backport detection), AD domain membership heuristic (krb5.conf/sssd.conf/nsswitch/binary presence, 2+ indicator threshold), sshd_config parsing (4 directives: PermitRootLogin, PermitEmptyPasswords, PasswordAuthentication, AllowAgentForwarding — case-insensitive, non-standard AuthorizedKeysFile detection), screen/tmux session hijacking (other users' attachable sockets, root session = critical) |
...
| 15 | Security Protections | AppArmor, SELinux, ASLR, mmap_min_addr, kptr_restrict, dmesg_restrict, perf_event_paranoid, ptrace_scope, seccomp, protected_symlinks/hardlinks, unprivileged_userns_clone, unprivileged_bpf_disabled, modules_disabled, module_sig_enforce, lockdown mode, grsecurity/PaX detection, loaded kernel module analysis (cross-reference against /lib/modules/ tree, non-standard module detection, DKMS-aware), permissive /dev/ device scan (world-writable + group-writable non-standard entries, one level deep) |
...
| 17 | Cloud Environment | Passive cloud detection (10 indicators: AWS EC2/ECS/Lambda/CodeBuild, GCP VM/Function, Azure VM/App Service, DigitalOcean, IBM Cloud — env vars, filesystem markers, DNS config, DMI fallback), cloud CLI tool presence (aws/gcloud/az/doctl), cloud credential file scanning across all home dirs (AWS/GCP/Azure, 6 paths), metadata route detection (/proc/net/route), cloud-init presence. Active IMDS harvest (gated behind active mode): AWS EC2 IMDSv1/v2 IAM credential theft + user data, AWS ECS task creds (169.254.170.2) + IMDS reachability, Lambda STS env vars, CodeBuild env.sh credential URI, GCP service account tokens + project SSH keys, Azure VM managed identity tokens (4 control planes) + user data, Azure App Service identity endpoint tokens, DigitalOcean metadata + user data, IBM Cloud instance identity → IAM token exchange. 2s HTTP timeouts, TLS support |
...
This is an active work-in-progress (v0.3.0). All 17 modules are functional. See [ARCHITECTURE.md](ARCHITECTURE.md) for design details and technical documentation.
