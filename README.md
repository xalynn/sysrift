...
| 11 | Installed Software `[A]` | Compilers, transfer tools, web servers, internal service detection (12 services including Duplicati/TeamCity/Portainer), Logstash config RCE surface (writable conf.d, ruby/exec directive detection, non-default path.config discovery, LS_USER run-as annotation), database services (MySQL/MariaDB/PostgreSQL), vulnerable software + userspace CVEs with backport detection, AD membership, sshd_config, session hijacking. Active: database default credential testing |
