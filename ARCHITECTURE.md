...
`Data.home_dirs` caches an array of home directories for users with valid login shells, parsed from `Data.passwd`. Shell filter excludes nologin, /false, /sync (both `/bin/` and `/usr/bin/` variants via `ends_with?`). Only directories that exist on disk are included. Catches service accounts with homes in `/opt`, `/srv`, etc. Consumed by mod_creds (database credential files, browser profiles, password manager databases) and mod_network (r-commands .rhosts check).
