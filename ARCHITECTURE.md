...
`Data.dir_exists?(path)` and `Data.file_exists?(path)` are safe-stat predicates that wrap `File.info?(path)` with `rescue File::Error → false`, then narrow on `info.directory?` / `info.file?`. Stdlib `Dir.exists?` and `File.exists?` raise `File::AccessDeniedError` on EACCES — the `?` variant swallows ENOENT but not permission failures. The helpers swallow both, which is the operationally correct semantics for an enumeration tool against a target with unknown permission topology. Adopted in `Data.home_dirs` itself and in `mod_creds` (Batch B credential checks plus `check_jenkins_creds`). The broader rollout to other modules is bundled with the unified filesystem walker (KANBAN card 47) since both touch the same call sites.
...
`FsWalker.run` is a single in-process descent from `/` that replaces the per-module `find` invocations historically used to enumerate SUID/SGID binaries, world-writable files, history files, SSH private keys, `.netrc` files, sensitive configs, backup files, recently-modified files, password vaults, terraform state, certificates and keystores, and log4j jars. One stat per entry; predicates emit into typed sets exposed through Data accessors.
...
Walker validated 100% SUID/SGID coverage match against `find / -perm -4000/-2000 -type f` ground truth. `mod_suid` migrated as the first consumer (drop-in — `Data.suid_files`/`Data.sgid_files` interface preserved). Migration of `mod_files`, `mod_writable`, and `mod_creds` to walker-backed accessors is the remaining work (KANBAN card 47 Sessions 2 and 3).
