...
src/constants.cr        -> colors, GTFOBINS, DANGEROUS_CAPS, INTERESTING_GROUPS, INTERPRETER_LIB_VARS, INTERESTING_PORTS, WALKER_*, etc.
src/output.cr           -> Out module, tee/hi/med/info/ok/blank/section helpers
src/runner.cr           -> read_file(), run(), run_lines()
src/fs_walker.cr        -> FsWalker.run, single-pass filesystem traversal feeding the Data layer typed sets
src/data.cr             -> Data module (lazy-cached system data, walker dispatch, active_mode flag)
...
Files are required in explicit dependency order. `src/fs_walker.cr` is required between `runner` and `data` because Data accessors delegate to it. Modules use glob require (`require "./src/modules/*"`) since they are all leaf nodes with identical dependency profiles. `src/menu.cr` is required last because `module_list` creates `Proc` literals referencing all `mod_*` functions.

## Design decisions

### Data collection layer (`src/data.cr`)

Modeled after linPEAS's startup variable pre-computation (`$suids_files`, `$mygroups`, `$sh_usrs`, etc.). The `Data` module provides lazy-cached properties -- expensive operations like `ps aux` and the in-process filesystem walk (see `src/fs_walker.cr`) run once on first access and are cached for the duration of execution. Properties that don't need external commands avoid spawning entirely: hostname via `System.hostname`, kernel version via `/proc/sys/kernel/osrelease`, environment variables via Crystal's `ENV.each`, mount table via `/proc/mounts`, process status via `/proc/self/status`, container detection via filesystem checks. Static file reads use native `File.read` via `read_file()` instead of spawning shell processes. SUID/SGID enumeration and the cluster of historical credential-hunting `find` invocations are handled by the unified walker; consumers read typed sets via `Data.suid_files`, `Data.sgid_files`, `Data.world_writable_dirs`, `Data.backup_files`, `Data.history_files`, etc.
...
`Data.home_dirs` caches an array of home directories for users with valid login shells, parsed from `Data.passwd`. Shell filter excludes nologin, /false, /sync (both `/bin/` and `/usr/bin/` variants via `ends_with?`). Only directories that exist on disk *and* are traversable by the current process are included — the `File::Info.executable?(home)` predicate enforces the search bit (exec on a directory == search permission). Without that filter, every consumer that stats `${home}/something` on an unreachable home (e.g., `/root` mode 700 from a non-root caller) crashes with `File::AccessDeniedError` from the underlying `stat()` syscall. Catches service accounts with homes in `/opt`, `/srv`, etc. Consumed by mod_creds (database credential files, browser profiles, password manager databases, Jenkins home discovery, .git-credentials/.gitconfig, terraform state walker, Terraform Cloud credentials, Docker registry config, host-side kubeconfig, AI assistant credential files, GPG private key material, certificate and keystore files) and mod_network (r-commands .rhosts check).

`Data.dir_exists?(path)` and `Data.file_exists?(path)` are safe-stat predicates that wrap `File.info?(path)` with `rescue File::Error → false`, then narrow on `info.directory?` / `info.file?`. Stdlib `Dir.exists?` and `File.exists?` raise `File::AccessDeniedError` on EACCES — the `?` variant swallows ENOENT but not permission failures. The helpers swallow both, which is the operationally correct semantics for an enumeration tool against a target with unknown permission topology. `Data.stat_safe(path) : File::Info?` is the same pattern but returns the full `File::Info` for callers that need size, owner_id, or mtime. Adopted across the credential-hunting helpers in mod_creds (24 stat sites covering owner checks, file gates, and dir gates), the sudoers and doas helpers in mod_sudo, the home-dir + `.ssh` gates in mod_users (the canonical mode-700 crash site from a non-owner caller), and the logrotate target gates in mod_writable. Remaining unwrapped stdlib calls live under `/etc/*` standard paths, `/proc`, `/sys`, and `/var/log` direct-children where the parent dir is canonically world-searchable.

No disk persistence between runs -- each foothold starts fresh. Re-run under each user context.

### Filesystem walker (`src/fs_walker.cr`)

`FsWalker.run` is a single in-process descent from `/` that enumerates SUID/SGID binaries, world-writable files, history files, SSH private keys, `.netrc` files, sensitive configs, backup files, recently-modified files, password vaults, terraform state, certificates and keystores, log4j jars, the credential-pattern content scan candidate set, and the log-file content scan candidate set. One stat per entry; predicates emit into typed sets exposed through Data accessors.

The walk is lazy — triggered the first time any walker-backed Data accessor is called. Modules that don't consume walker results never pay its cost. After completion the result hash and a separate target-realpath → incoming-symlinks alias map are cached on Data for the rest of the session. Returns a `NamedTuple(results: Hash(Symbol, Array(String)), aliases: Hash(String, Array(String)))`.

Skip set is the only depth defense — the walk descends `/` unbounded. `WALKER_SKIP_BASENAMES` (15 entries) prunes build/dependency caches anywhere in the tree (`node_modules`, `venv`, `.cache`, `.cargo`, `.rustup`, `.npm`, `site-packages`, `gems`, `snap`, `flatpak`, etc.). `WALKER_SKIP_PATHS` (9 entries) prunes pseudo filesystems and container layer storage at the root level (`/proc`, `/sys`, `/dev`, `/run`, `/snap`, `/var/lib/docker`, `/var/lib/lxd`, `/var/lib/containers`, `/var/lib/flatpak`). `WALKER_SKIP_FSTYPES` cross-references `Data.mounts` to skip network mounts (NFS, SMB, FUSE network) and overlay/squashfs — both a perf win and an OPSEC win against `find /` traversing remote shares. `WALKER_MAX_ENTRIES = 10_000_000` is a hard ceiling for pathological filesystems that bypass the skip set; cleaner abort than OOM.

`.m2` and `Trash` deliberately absent from the skip set despite being heavy on dev hosts. `~/.m2/settings.xml` at depth 1 holds Maven repository credentials (real cred location on Java targets). `~/.local/share/Trash/files/` is occasionally seeded with vectors on HTB. Coverage outweighs the marginal perf cost.

One stat per entry uses direct `LibC.lstat` — the symbol is in Crystal's `lib_c` bindings, no new binding code required. Crystal's `File::Info` exposes `same_file?(other)` for inode comparison but does not publicly expose `dev` or `inode`; the underlying `@stat : LibC::Stat` is private to the system module. `File::Info.new(stat)` from a self-issued lstat result wraps the same struct and provides the convenient predicate API (`flags.set_user?`, `permissions.other_write?`, `type.symlink?`). The visited set is `Set({UInt64, UInt64})` keyed on `{st_dev, st_ino}` — inode-level dedup catches bind-mount cycles that path-only dedup would loop through.

`follow_symlinks: false` everywhere. Symlinks with targets inside the walk are recorded in the alias map for `Data.symlink_aliases` consumers; symlinks with targets outside the walk (broken, `/proc`, `/sys`, skipped subtrees) are dropped. The walker does not mutate result strings — the alias map lives separately so consumers iterating `Data.suid_files` get clean filesystem paths suitable for `File.info?` / `File.open` calls, and consumers that want alias context can opt-in via `Data.symlink_aliases[path]?`.

**Predicate categories.** Per-file predicates emit into typed sets:
- Permission bits → `:suid`, `:sgid`, `:world_writable_files`
- Backup extensions → `:backup_files` (capped at 100)
- Recent mtime (≤10 min) → `:recent_files` (capped at 20)
- Exact basename or regex match → `:ssh_keys`, `:netrc_files`, `:history_files`, `:sensitive_configs`
- Vault / tfstate / cert extensions → `:password_vaults`, `:tfstate_files`, `:cert_keystores`, `:cert_pemkeys`
- log4j basename → `:log4j_jars`
- Generic credential-extension whitelist (`WALKER_CRED_SCAN_EXTS`, 15 extensions) → `:cred_scan_files` (capped at 100K — bound is generous to avoid silent coverage regression on systems where heavy extension-bearing subtrees come before `/etc` in traversal order)
- Path-prefix predicate (`WALKER_LOG_DIR_PATHS = ["/var/log/", "/var/logs/"]`) → `:log_files` (capped at 10K)

Per-directory predicate emits into `:world_writable_dirs` (capped at 30).

Two bounded sub-walks complete the surface: PATH dirs to depth 2 (`.sh` scripts and broken symlinks — both PATH-hijack windows) and `/lib/modules` to depth 8 (non-root-owned `.ko` files — LKM rootkit window). Result sets `Data.path_sh_scripts`, `Data.path_broken_symlinks`, and `Data.weak_kernel_modules` are populated but have no consumer module yet.

A status-line fiber prints a single in-place updating line via `\r` carriage return at `WALKER_STATUS_INTERVAL` (2.5s) cadence, suppressed when `STDOUT.tty?` is false so piped/redirected output stays clean. The final completion line emits via `Out.write` so it lands in both the live output and the tee'd log. `Channel(Nil)` + `select`/`timeout` drive the fiber loop; `Fiber.yield` every ~1024 entries from the walk yields cooperatively so the status fiber gets to redraw.

**Bucket-dispatch consumer pattern (mod_creds cred-pattern scan).** `Data.cred_scan_files` is a candidate pool for content-pattern scans across multiple `(dir, scan-type)` combinations. mod_creds buckets the candidates in one outer pass over the set — for each path, it walks the prefix maps (`{prefix → dir}`) once, dispatching into `cred_buckets`, `js_buckets`, `secret_buckets` keyed by dir. Helpers (`scan_cred_keywords`, `scan_secret_patterns`) then iterate per-bucket arrays and apply content regex per file. Crystal's regex engine raises `ArgumentError` on non-UTF-8 input — the helpers rescue it, and a NUL-byte check on the first 4KB skips binary blobs before the regex sees them.

**Walker-backed module consumers**: `mod_suid` (`Data.suid_files`, `Data.sgid_files`), `mod_files` (`Data.sensitive_configs`, `Data.backup_files`, `Data.recent_files`, `Data.log_files`), `mod_writable` (`Data.world_writable_dirs`), `mod_creds` (`Data.history_files`, `Data.ssh_private_keys`, `Data.netrc_files`, `Data.log4j_jars`, `Data.password_vault_files`, `Data.tfstate_files`, `Data.cert_keystore_files`, `Data.cert_pemkey_files`, `Data.cred_scan_files`).

Walker SUID/SGID output matches `find / -perm -4000/-2000 -type f` ground truth on a representative dev workstation (3.1M filesystem entries, ~1.09M post-skip-set walker visits). Module 4 (Credential Hunting) and module 14 (Interesting Files) complete in ~30 seconds end-to-end on the same target.
...
All command strings passed to `run()` use hardcoded inputs, not filesystem-derived or user-supplied values. Module logic that needs to apply patterns to discovered files reads via `read_file()` and matches in-process rather than interpolating paths into a shell command. `Data.sudo_l` bypasses `run()` entirely -- it uses `Process.new` with stdin closed and a timeout (see data collection layer above).
...
**Data layer caching.** Commands duplicated across modules run once: `id`, `ps aux`, `ss -tulpn`, `/etc/passwd`, `/etc/shadow`, `/etc/sudoers`, `/etc/ssh/sshd_config`, `/etc/resolv.conf`, `/etc/group`, `/proc/self/status`, `/proc/mounts`. Package manager queries for runtime CVE checks (`runc`, `containerd`, `logrotate`) are one spawn each, cached for the session.

**Unified filesystem walker.** SUID/SGID enumeration, the per-module credential-hunting `find` invocations, and the recursive Dir.each_child walks for password vaults / tfstate / certificates are all served by a single in-process descent from `/` (see `src/fs_walker.cr` and the Filesystem walker section above). One stat per entry via direct `LibC.lstat`, predicates emit into typed sets exposed through Data accessors. The credential-pattern content sweep that used to shell `grep -rIilE` per dir is now an in-process scan over walker-collected candidate paths.

**In-process parsing replaces shell pipelines.** Walker-backed candidate sets feed in-process content regex matching for credential keywords (`scan_cred_keywords`), API key signatures (`scan_secret_patterns`), and log keywords (`mod_files` log scan). Crystal's regex engine requires UTF-8 input — both content scanners use a NUL-byte binary-skip + `ArgumentError` rescue to handle non-UTF-8 file content (Latin-1 configs, embedded binary blobs in `.json`). `/proc/net/tcp` hex parsing replaces ss/netstat for r-service port detection. `/proc/modules` + `Dir.glob` on `/lib/modules/` replaces `lsmod` and `modinfo`. Chroot, FD, and environ analysis reads `/proc/[pid]/root`, `fd/`, and `environ` directly instead of `lsof` or `fuser`. Firewall enumeration reads saved rule files instead of `iptables -L` (requires root). Container namespace isolation reads `/proc/1/comm`, `/proc/net/dev`, and hostname directly. Container pivot enumeration reads `/proc/net/fib_trie` and `/proc/net/arp` directly instead of `ip addr` and `arp -a`. Systemd PATH reads `/proc/1/environ` instead of `systemctl show-environment`. Writable systemd unit detection uses a two-level `Dir.each_child` walk instead of `find -writable`; ExecStart parsing shares the same iteration.

**Streaming and direct spawn.** `Data.sudo_l` spawns sudo directly via `Process.new` with stdin closed (prevents hangs on non-TTY shells) instead of routing through `/bin/sh`. `aureport --tty` uses streaming `Process.new` with line-by-line consumption and early break. Kubernetes RBAC and resource enumeration spawns kubectl (1-5 invocations, only when kubectl is present and inside a K8s pod); the path is resolved once via `Process.find_executable` and passed to both functions. ACL enumeration spawns `getfacl` via `Process.new` with stream read and 500KB byte cap. Kerberos keytab principal enumeration spawns `klist -k` once per readable keytab; output parsed in-process with header/separator row filtering by KVNO-column numeric check.
