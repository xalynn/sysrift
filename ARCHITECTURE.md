...
`Data.dir_exists?(path)` and `Data.file_exists?(path)` are safe-stat predicates that wrap `File.info?(path)` with `rescue File::Error → false`, then narrow on `info.directory?` / `info.file?`. Stdlib `Dir.exists?` and `File.exists?` raise `File::AccessDeniedError` on EACCES — the `?` variant swallows ENOENT but not permission failures. The helpers swallow both, which is the operationally correct semantics for an enumeration tool against a target with unknown permission topology. `Data.stat_safe(path) : File::Info?` is the same pattern but returns the full `File::Info` for callers that need size, owner_id, or mtime. Adopted in `Data.home_dirs` and across the GPG, certificate, Jenkins, terraform-state, and credential-content-scan helpers in mod_creds. Other modules retain unwrapped stdlib calls at ~13 sites — broader rollout pending.
